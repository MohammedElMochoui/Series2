	...
	visit (asts){
		case node n: ({
			...
			if (nodeMass > minMass && n.src? && loc location := n.src) {
				if (n.modifiers?) {
					println( typeOf(n.modifiers));
				}
				node cleaned = unsetRec(n);
				str hash = toString(removeIdentifiers(cleaned)) + modifiers;
...
map [str, rel[node, loc]] removeSubClones(map [str, rel[node, loc]] cloneClasses){
	map [str class, rel[node n, loc l] nodes] cloneClassesCleaned = ();
	
	for (class <- cloneClasses) {
		rel[node n, loc l] clones = cloneClasses[class];
		rel[tuple[node n, loc l] first, tuple[node n, loc l] second] clonePairs = makeClonePairs(clones);
		
		for (pair <- clonePairs) {
			bool addition = true;
			loc src1 = pair.first.l;
			loc src2 = pair.second.l;
		
			for (restClass <- delete(cloneClasses, class)){
				rel[node n, loc l] restClones = cloneClasses[restClass];
				rel[tuple[node n, loc l] first, tuple[node n, loc l] second] restPairs = makeClonePairs(restClones);
				
				for (restPair <- restPairs){
					loc src3 = restPair.first.l;
					loc src4 = restPair.second.l;
					
					if ((checkIfPartOf(src1, src3) == 2 && checkIfPartOf(src2, src4) == 2) ||
					(checkIfPartOf(src1, src4) == 2 && checkIfPartOf(src2, src3) == 2)
					){
						addition = false;
						break;					
					}
			...
			if (addition) {
				if (cloneClassesCleaned[class]?) cloneClassesCleaned[class] += {pair.first, pair.second};
				else cloneClassesCleaned[class] = {pair.first, pair.second};
			}
		}
	}
	
	return cloneClassesCleaned;
}


void runDetection(){
	list[Declaration] projectAST = getASTs(|project://smallsql0.21_src|);
	map[str hashes, lrel[node n, loc l] nodes] nodeMap = catagorizeNodes(projectAST, 25);
	...
	//map [str class, rel[node n, loc l] nodes] cloneClassesCleaned = removeSubClones(cloneClasses);
	
	//println(size(toList(cloneClassesCleaned.class)));
	//rel[node n, loc l] nodes = {};
	//
	//for (class <- cloneClassesCleaned) {
	//	rel[node n, loc l] clones = cloneClassesCleaned[class];
	//	for (clone <- clones) {
	//		nodes += clone;
	//	}
	//}
